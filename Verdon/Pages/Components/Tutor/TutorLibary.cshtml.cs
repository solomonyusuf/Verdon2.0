...
namespace Verdon.Pages.Components.Tutor
{
    ...
    public class TutorLibaryModel : PageModel
    {
        ...
        public TutorLibaryModel(UserManager<VerdonUser> manager,UploadControllers upload,IToastNotification toast, ApplicationDbContext context)
        {
            _manager = manager;
            _upload = upload;
            _toast = toast;
        ...
        public async Task OnGet()
        {
            try
            {
                ...
                materials = await _context.Libaries.OrderByDescending(x=> x.Id).Where(x=> x.VerdonUserId == user.Id).OrderByDescending(x => x.Id).ToListAsync();
